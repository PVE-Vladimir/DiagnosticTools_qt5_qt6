...
void DataQueryEngine::getData(){
    ...
    if ( GetProcessMemoryInfo(  m_hProcess, &pmc, sizeof(pmc)) ){
        double residentSet1 = pmc.WorkingSetSize/1024;
        if (residentSet1>DiagnostictoolsPlugin::PeakRss){
            DiagnostictoolsPlugin::PeakRss= residentSet1;
        }
...
//        qDebug() << "pmc.PeakPagefileUsage" << pmc.PeakPagefileUsage/1024;
        ...
        emit ramDataUpdated(residentSet1,
                            pmc.PagefileUsage/1024, pmc.PeakPagefileUsage/1024),
                            DiagnostictoolsPlugin::PeakRss;
    }
}
#elif defined Q_OS_LINUX
void DataQueryEngine::getData(){
    ...
    if (vmUsage>DiagnostictoolsPlugin::PeakVsize){
        DiagnostictoolsPlugin::PeakVsize= vmUsage;
    }
    if (residentSet>DiagnostictoolsPlugin::PeakRss){
        DiagnostictoolsPlugin::PeakRss= residentSet;
    }
    emit ramDataUpdated(residentSet, vmUsage, DiagnostictoolsPlugin::PeakVsize, DiagnostictoolsPlugin::PeakRss);
...
void DataQueryEngine::getData(){
    ...
    if (KERN_SUCCESS == task_info(task,
                                  ...
                                  &info_count))
    {
        double residentSet1 = vm_info.resident_size / 1024.0;
        if (residentSet1>DiagnostictoolsPlugin::PeakRss){
            DiagnostictoolsPlugin::PeakRss= residentSet1;
        }
        emit ramDataUpdated(
